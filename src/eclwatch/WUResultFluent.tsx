...
export interface WUResultProps {
    result: Result;
}
export const WUResult: React.FunctionComponent<WUResultProps> = ({
    ...
    React.useEffect(() => {
        result.refresh().then(() => {
            setColumns(result.fields().map((f): IColumn => {
                return {
                    key: f.name,
                    name: f.name,
                    fieldName: f.name,
                    minWidth: f.charWidth() * 12,
                    maxWidth: f.charWidth() * 18,
                    isResizable: true
                    // name: "File Type",
        ...
        result.fetchRows(0, 25).then(rows => {
            setRows(rows.map((r, i) => ({ key: i, ...r })));
        });
    }, []);
...
class WUResult2 extends React.Component<object, IDetailsListDocumentsExampleState> {
